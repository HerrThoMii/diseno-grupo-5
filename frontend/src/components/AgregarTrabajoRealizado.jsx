...
import { crearTrabajoPublicado, listarGrupos, listarAutores, listarTiposTrabajoPublicado } from '../services/api';
...
export default function AgregarTrabajoRealizado({ isOpen, onClose, onAdd }) {
  ...
  useEffect(() => {
    ...
    Promise.all([listarGrupos(), listarAutores(), listarTiposTrabajoPublicado()])
      ...
      .catch(() => { if (!mounted) return; setGrupos([]); setAutores([]); setTipos([]); });
    return () => { mounted = false };
  }, []);
  ...
  const handleCreateAuthor = () => {
    ...
    crearAutor({ nombre: newAuthor.nombre.trim(), apellido: newAuthor.apellido.trim() })
      .then(created => {
